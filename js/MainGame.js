...
class MainGame {
    ...
    startLogic() {
        ...
        this.actualLevel = difficulty.EASY;
        this.points = 0;
    ...
    updateGameLogic() {
        ...
        this.addPoints(this.pointsPerUpdate);

        /**Debug performance*
    	let oldLogic = this.lastLogic
    	this.lastLogic = (new Date()).getMilliseconds();
	    console.log("lastLogic: "+this.lastLogic+ " logicInterval: "+(this.lastLogic-oldLogic)+" logicDuration: "+(this.lastLogic-logicStart));//*/
    }
    ...
    genObstacle() {
        ...
        if (probabilityOfItem >= 60) {
            ...
            for (var i = 0; i < 10; i++) {
                ...
                posX = posX.toFixed(3);
                if (this.arrayObjects.length <= 0 || this.checkPosition(posX, posY, this.arrayObjects)) {
                    let selectedAnimation = animations.obstacle_objects[Math.floor(Math.random() * animations.obstacle_objects.length)];
                    let obstacle = new HitObject(this.ctx, posY, posX,
                        selectedAnimation, this.character, false, this.objectCelerity);
                    obstacle.resize(this.canvasWidth, this.canvasHeight);
                    this.arrayObjects.push(obstacle);
                    return true;
                }
            ...
            for (let i = 0; i < 10; i++) {
                ...
                posX = posX.toFixed(3);
                if (this.arrayObjects.length <= 0 || this.checkPosition(posX, posY, this.arrayObjects)) {
                    let selectedAnimation = animations.cut_objects[Math.floor(Math.random() * animations.cut_objects.length)];
                    let obstacle = new HitObject(this.ctx, posY, posX,
                        selectedAnimation, this.character, true, this.objectCelerity);
                    obstacle.resize(this.canvasWidth, this.canvasHeight);
                    this.arrayObjects.push(obstacle);
                    return true;
                }
    ...
    nextLevel() {
        if (this.actualLevel == difficulty.EASY && this.points > levelPoints.MEDIUM) {
            this.actualLevel = difficulty.MEDIUM;
        ...
        else if (this.actualLevel == difficulty.MEDIUM && this.points > levelPoints.HARD) {
            this.actualLevel = difficulty.HARD;
        ...
        else if (this.actualLevel == difficulty.HARD && this.points > levelPoints.HELL) {
            this.actualLevel = difficulty.HELL;
    ...
    endGame() {
        sessionStorage.setItem("lastScore", this.points);
    ...
    repaint() {
    	...
    	console.log("lastPaint: "+this.lastPaint+ " paintInterval: "+(this.lastPaint-oldPaint)+" paintDuration: "+(this.lastPaint-paintStart));//*/
    }
    resizeCanvas() {
        ...
        //paint lines
        this.lineMax.css({ top: (canvasOffset.top + this.canvasHeight * this.character.relativePosYMax), left: canvasOffset.left });
        this.lineMin.css({ top: (canvasOffset.top + this.canvasHeight * this.character.relativePosYMin + this.character.height), left: canvasOffset.left });
        this.lineMax.attr("width", this.canvasWidth);
        this.lineMin.attr("width", this.canvasWidth);
